...
class MazeView2D:
    ...
    def __init__(self, maze_name="Maze2D", maze_file_path=None,
        ...
        # Create the Robot
        self.__robots = [{'pos': self.entrance, 'colour': (0, 0, 150)}]
        ...
        if self.__enable_render is True:
            ...
            # show the robot
            self.__draw_robot(self.__robots[0])
    ...
    def add_robot(self, pos, colour=(150, 0, 0)):
        self.__robots.append({'pos': pos, 'colour': colour})
        
        # show the robot
        self.__draw_robot({'pos': pos, 'colour': colour})
    ...
    def move_robot(self, dir, number = 0):
        ...
        robot = self.__robots[number]
        if self.__maze.is_open(robot['pos'], dir) and self.__not_blocked_by_robot(robot['pos'], dir):

            # update the drawing
            self.__draw_robot(robot, transparency=0)

            # move the robot
            self.__robots[number]['pos'] += np.array(self.__maze.COMPASS[dir])
            # if it's in a portal afterward
            if self.maze.is_portal(self.robot):
                self.__robots[number]['pos'] = np.array(self.maze.get_portal(tuple(self.robot)).teleport(tuple(self.robot)))
            self.__draw_robot(robot, transparency=255)

    def __not_blocked_by_robot(self, current_pos, dir):
        new_pos = current_pos + np.array(self.__maze.COMPASS[dir])
        blocked = False
        for r in self.__robots:
            blocked |= r['pos'][0] == new_pos[0] and r['pos'][1] == new_pos[1]

        return not blocked

    def reset_robot(self):
        self.__draw_robot(self.__robots[0], transparency=0)
        self.__robots[0]['pos'] = np.zeros(2, dtype=int)
        self.__draw_robot(self.__robots[0], transparency=255)
    ...
    def __draw_robot(self, robot, transparency=255):

        if self.__enable_render is False:
            return
        
        x = int(robot['pos'][0] * self.CELL_W + self.CELL_W * 0.5 + 0.5)
        y = int(robot['pos'][1] * self.CELL_H + self.CELL_H * 0.5 + 0.5)
        r = int(min(self.CELL_W, self.CELL_H)/5 + 0.5)

        pygame.draw.circle(self.maze_layer, robot['colour'] + (transparency,), (x, y), r)
...
class Maze:
    ...
    def change_wall(self, cell_id, dir, wall):
        opposite_point = self.__get_next_cell(cell_id, dir)
        if opposite_point == None:
            return

        cell = self.maze_cells[cell_id[0], cell_id[1]]
        if not wall:
            if dir is "N":
                cell |= 0x1
            if dir is "E":
        ...
        if wall:
            ...
            if dir is "W":
                cell &= ~0x8
        self.maze_cells[cell_id[0], cell_id[1]] = cell

        opposite_wall = self.__get_opposite_wall(dir)[0]
        cell = self.maze_cells[opposite_point[0], opposite_point[1]]
        dir = opposite_wall
        if not wall:
            if dir is "N":
                cell |= 0x1
        ...
        if wall:
            ...
            if dir is "W":
                cell &= ~0x8
        self.maze_cells[opposite_point[0], opposite_point[1]] = cell

    def __get_next_cell(self, cell_id, dir):
        x = cell_id[0] + self.COMPASS[dir][0]
        y = cell_id[1] + self.COMPASS[dir][1]
        return (x,y) if self.is_within_bound(x, y) else None
